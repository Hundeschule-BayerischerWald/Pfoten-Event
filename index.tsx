...
const api = {
    ...
    addEvent: async (newEventData: Omit<Event, 'id' | 'booked_capacity' | 'total_capacity'> & {totalCapacity: number, date: Date}): Promise<Event> => {
        const { data, error } = await supabase.from('events').insert({
            ...
            infotext: newEventData.infotext,
            booked_capacity: 0
        }).select().single();
    ...
    updateEvent: async (eventId: string, updatedEventData: Partial<Omit<Event, 'id' | 'date' | 'total_capacity' | 'booked_capacity'>> & { date?: Date; totalCapacity?: number }): Promise<Event> => {
        ...
        // Ensure infotext is included in the payload
        if (updatedEventData.infotext !== undefined) {
             payload.infotext = updatedEventData.infotext;
        }
        // Remove description if it exists in the payload to avoid errors if the column doesn't exist or is not used
...
const EventItem = ({ event, onSelect, isSelected, isLocked }) => {
    ...
    const month = new Intl.DateTimeFormat('de-DE', { month: 'short' }).format(event.date).toUpperCase().replace('.', '');
    const weekday = new Intl.DateTimeFormat('de-DE', { weekday: 'short' }).format(event.date).toUpperCase().replace('.', '');
    const timeRange = formatTime(event.date); // Assuming formatTime returns "HH:MM Uhr"
    ...
    return html`
        <div 
            class=${`event-card ${categoryClass} ${isSelected ? 'selected' : ''} ${isDisabled ? 'disabled' : ''}`}
        ...
        >
            ...
            <div class="event-card-status">
                ${isLocked 
                    ? html`<span class="status-badge locked">Vergangen</span>` 
                    : isFull 
                        ? html`<span class="status-badge full">Leider Ausgebucht</span>` 
                        : html`<span class="status-badge available">${remaining} ${remaining === 1 ? 'Platz' : 'Plätze'} noch frei</span>`
                }
            </div>
        </div>
    `;
...
const EventFormModal = ({ event, onSave, onClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const eventData = {
            ...
            infotext: formData.infotext || null,
        };
        onSave(eventData);
    };
                ...
                <form onSubmit=${handleSubmit}>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                         ...
                         <div class="form-group">
                            <label for="infotext">Beschreibung</label>
                            <textarea id="infotext" name="infotext" value=${formData.infotext} onInput=${handleChange} placeholder="Zusätzliche Infos zum Event..."></textarea>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            ...
            try {
                ...
                // Sende die E-Mail immer, auch wenn keine zukünftigen Events mehr vorhanden sind (z.B. nach Stornierung aller Events)
                await supabase.functions.invoke('send-smtp-email', {
                    body: {
                        type: 'update-booking',
...
const App = () => {
    ...
    useEffect(() => {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
            ...
            if (session) { // If a session is established (login successful)
                ...
                // After login, prefer admin view unless a specific view is in URL
                const params = new URLSearchParams(window.location.search);
                const viewParam = params.get('view');
    ...
    // Special full-screen views that don't share the main header
    if (view === 'monitor') {
        if (!session) {
             return html`
                <div class="monitor-login-wrapper">
                    <${AdminLoginModal} 
                        ...
                        initialError="Bitte einloggen für den Monitor-Modus" 
                    />
                </div>
             `;
        }
