...
const FROM_EMAIL = 'Canicanum <anmeldungen@pfotencard.hs-bw.com>';
const REPLY_TO_EMAIL = 'info@hs-bw.com';
const EMAIL_HEADER_IMAGE_URL = 'https://canicanum.de/wp-content/uploads/2026/03/Emailheader.png';
...
function createEmailHtml(title: string, customerName: string, bookingId: string, events: any[], manageUrl: string, type: string) {
  const eventsHtml = events.length > 0 
    ...
    : '<p style="text-align: center; padding: 1rem; background-color: #f8f9fa; border-radius: 8px; color: #666;">Du hast aktuell keine offenen Termine.</p>';
  ...
  return `
    ...
    <img src="${EMAIL_HEADER_IMAGE_URL}" alt="Canicanum Banner" style="max-width: 100%; height: auto; display: block; margin: 0 auto;">
    <div class="content">
        ...
        <div style="text-align: center; margin: 25px 0;">
          <a href="${manageUrl}" target="_blank" style="background-color: #A4A68A; color: white; padding: 12px 25px; text-decoration: none; border-radius: 12px; font-weight: bold; display: inline-block;">
        ...
        <p style="margin-top: 20px; font-size: 12px; color: #888; text-align: center;">
          ...
          dein Team von Netzwerk Canicanum
        </p>
    </div></div></body></html>
  `;
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'no-show') {
        ...
        subject = type === 'new-booking' ? 'Deine Buchungsbestätigung für Canicanum' : 'Deine Buchung wurde aktualisiert';
